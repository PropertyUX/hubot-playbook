 ...
 * message and current module. Storing and finding records is handled by the
 * brain storage adapter - therefore requires a storage adapter to be loaded.
 *
 ...
 *
 * @param {Robot}  robot                  Bot instance
 ...
 *
 * @todo Abort at constructor if storage adapter not loaded
*/
class Transcript extends Base {
  constructor (...args) {
    ...
    // we will store our eventListeners as eventListeners.[dialogue_id][path_id][event_name]
    this.eventListeners = {}
  }

  /**
   * Record given event details in array, store via brain if configured to.
   *
  ...
  */
  recordEvent (event, ...args) {
    ...
    this.robot.brain.store('transcripts', record)
  ...
  recordDialogue (dialogue) {
    ...
    dialogue.on('path', (path) => {
      pathEvents.map((event) => {
        // store each eventListener
        _.set(this.eventListeners,
          `[${dialogue.id}][${path.id}][${event}]`,
  ...
  */
  recordScene (scene) {
    ...
    scene.on('exit', (...args) => {
      ...
      // We need to remove the eventListeners for this scene.
      // Couldn't find a definitive way to know whether all events
      // had fired, so we use setTimeout to them chance to complete.
      setTimeout(() => {
        // get the id of the dialogue that is exiting
        const dialogueId = _.get(args.filter(e => e.hasOwnProperty('dialogue')), '0.dialogue.id')
        if (dialogueId) {
          // iterate each path we that created
          _.forEach(this.eventListeners[dialogueId], (pathEvents, pathId) => {
            // iterate the listeners for this path
            _.forEach(pathEvents, (listener, event) => {
              this.robot.events.removeListener(event, listener)
            })
          }) 
        }
      }, 500) // wait for all emitted events to do their thing
    })
  }
  ...
  */
   ...
   * @return {Promise}             Resolves to matched records / selected values
   *
   * @example
  ...
  */
  async findRecords (subsetMatch, returnPath) {
    let found = await this.robot.brain.find('transcripts', subsetMatch)
