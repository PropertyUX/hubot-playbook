...
class Scene extends Base {
  ...
  */
  ...
  processEnter (context, done) {
    ...
    this.timeoutListener = dialogue.on('timeout', (lastRes, other) => {
      return this.exit(lastRes, 'timeout')
    })
    this.endListener = dialogue.on('end', (lastRes) => {
  ...
  */
  exit (res, status = 'unknown') {
    ...
    if (this.engaged[participants] != null) {
      ...
      this.robot.events.removeListener('timeout', this.timeoutListener)
      this.robot.events.removeListener('end', this.endListener)
