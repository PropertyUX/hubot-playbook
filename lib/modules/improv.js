...
function parse (context, match) {
  ...
  if (pathmatches.length === 0 ||
    ...
    pathmatches[0][0] === match) return context.strings
  ...
  return context.strings.map((string) => {
    ...
    // Allow expressions to refer to other expressions...
    // e.g. send: ${this.site.defaultResponse}
    // where
    // this.site.defaultResponse == 'Ask ${this.building.managers} for some info'
    return parse(_.assign({}, context, {strings: [string]}), pathmatches[0][0])[0]
  })
}
...
function render (string, match, callData) {
  try {
    let template = new Function(`if (!${match[1]}) throw new Error\nreturn \`${match[0]}\``) // eslint-disable-line
