 ...
 * @param {boolean}  [options.fallback]       Optional string for empty/missing bits[].send
...
class Outline extends Base {
  constructor (robot, bits, ...args) {
    ...
    this.defaults({ 
      setupScenes: true,
      fallback: process.env.OUTLINE_FALLBACK || "(content is empty or missing)"
    })
  ...
  bitCallback (bit, res) {
    res.bit = bit
    let sends = _.isEmpty(bit.send)
      ? _.castArray(this.config.fallback)
      : _.castArray(bit.send)
